...
function GoogleCallbackContent() {
  ...
  useEffect(() => {
    const handleGoogleCallback = async () => {
      try {
        ...
        const loginRes = await authFetch(`${API_BASE}/api/auth/google/login/`, {
          method: "POST",
  ...
  return (
    <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "100vh", backgroundColor: "#f5f5f5", flexDirection: "column", gap: "20px" }}>
      <div style={{ textAlign: "center", padding: "40px", backgroundColor: "white", borderRadius: "12px", boxShadow: "0 4px 20px rgba(0,0,0,0.1)", maxWidth: "400px" }}>
        ...
        <h2 style={{ color: messageType === "success" ? "#22c55e" : messageType === "error" ? "#dc2626" : "#2563eb", margin: "0 0 10px", fontSize: "18px" }}>
          {messageType === "success" ? "نجاح" : messageType === "error" ? "خطأ" : "جاري المعالجة"}
        </h2>
        <p style={{ color: "#666", fontSize: "15px", margin: "0 0 15px", lineHeight: "1.6" }}>{message}</p>
        <p style={{ color: "#999", fontSize: "12px", margin: 0 }}>سيتم تحويلك تلقائياً...</p>
      </div>
    </div>
  );
}
