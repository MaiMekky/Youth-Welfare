...
function buildErrors(
  ...
  if (!formData.name.trim()) errors.name = "اسم الاتحاد مطلوب";
  else if (formData.name.trim().length < 5) errors.name = "يجب أن يكون الاسم 5 أحرف على الأقل";

  if (!formData.description.trim()) errors.description = "وصف الاتحاد مطلوب";
  else if (formData.description.trim().length < 10) errors.description = "يجب أن يكون الوصف 10 أحرف على الأقل";
...
function CreateUnionContent() {
  ...
  const handleCommitteeChange = (index: number, role: "head" | "assistant", value: string) => {
    setCommittees((prev) => {
      const updated = [...prev];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push(`/Family-Faclevel/unions/${data.family_id}`);
    } catch (error) {
          ...
          <div className={styles.formGrid}>
            <div className={styles.formGroup}>
              ...
              <input
                ...
                onChange={(e) => handleInputChange("president_nid", e.target.value.replace(/\D/g, ""))}
                placeholder="أدخل رقم الهوية"
                maxLength={14}
              />
              {errors.president_nid && <span className={styles.errorMsg}>{errors.president_nid}</span>}
            </div>
            ...
            <div className={styles.formGroup}>
              ...
              <input
                ...
                onChange={(e) => handleInputChange("vice_president_nid", e.target.value.replace(/\D/g, ""))}
                placeholder="أدخل رقم الهوية"
                maxLength={14}
              />
              {errors.vice_president_nid && <span className={styles.errorMsg}>{errors.vice_president_nid}</span>}
            </div>
          </div>
        </div>
                  ...
                  <div className={styles.committeeSection}>
                    ...
                    <div className={styles.formGroup}>
                      ...
                      <input
                        ...
                        onChange={(e) => handleCommitteeChange(index, "head", e.target.value.replace(/\D/g, ""))}
                        placeholder="UID"
                      />
                      {cErr.head_uid && <span className={styles.errorMsg}>{cErr.head_uid}</span>}
                    </div>
                  </div>
                  ...
                  <div className={styles.committeeSection}>
                    ...
                    <div className={styles.formGroup}>
                      ...
                      <input
                        ...
                        onChange={(e) => handleCommitteeChange(index, "assistant", e.target.value.replace(/\D/g, ""))}
                        placeholder="UID"
                      />
                      {cErr.assistant_uid && <span className={styles.errorMsg}>{cErr.assistant_uid}</span>}
                    </div>
                  </div>
                </div>
              );
...
export default function CreateUnionPage() {
  return (
    <Suspense fallback={<div style={{ padding: "2rem", textAlign: "center" }}><p>جاري التحميل...</p></div>}>
      <CreateUnionContent />
    </Suspense>
  );
}
