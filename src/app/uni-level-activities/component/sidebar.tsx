    ...
    <>
      <aside id="sidebar" className={`sidebar ${isOpen ? "open" : ""}`}>
          ...
          <div className="logo-container">
            <div className="logo-wrapper">
              <div className="logo-circle">
                <Image src={logo} alt="شعار الجامعة" className="sidebar-logo" width={96} height={96} priority />
              </div>
            </div>
          </div>
          <h2 className="sidebar-title">إدارة الفعاليات</h2>
          <p className="sidebar-subtitle">جامعة العاصمة - قسم خدمات الطلاب</p>
          <button className="sidebar-close-btn" onClick={() => setIsOpen(false)} aria-label="إغلاق القائمة">
            <X size={20} />
          </button>
        </div>
        ...
        <nav className="nav">
          ...
          <button className={pathname === "/uni-level-activities/plans" ? "active" : ""} onClick={() => go("/uni-level-activities/plans")}>
            <Users size={18} /><span>الخطط</span>
          </button>
          <button className={pathname === "/uni-level-activities/uni-level-faculty-events" ? "active" : ""} onClick={() => go("/uni-level-activities/uni-level-faculty-events")}>
            <Bell size={18} /><span>فعاليات الكليات</span>
          </button>
        </nav>
