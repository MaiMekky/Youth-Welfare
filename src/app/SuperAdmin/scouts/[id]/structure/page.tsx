...
export default function StructurePage() {
  ...
  const fetchStructure = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        const url = buildApiUrl(API_URL, "/api/dept/clan_structure/", { clan_id: clanId });
        const res = await authFetch(url);
        
        if (!res.ok) {
          showToast("فشل في جلب الهيكل التنظيمي", "error");
          return;
        }
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.breadcrumb}>
        ...
        <span className={styles.breadcrumbSep}>/</span>
        <span className={styles.breadcrumbCurrent}>الهيكل التنظيمي</span>
      </div>
      ...
      <header className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <button
            ...
            onClick={() => router.push(`/uni-level-scouts/${clanId}`)}
          >
            <ArrowRight size={16} />
            العودة
          </button>
