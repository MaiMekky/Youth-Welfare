  ...
  return (
    ...
    >
      ...
      <div className={`${styles.avatar} ${styles[`avatar_${roleColor}`]}`}>
        {isLeader ? <Crown size={compact ? 14 : 18} /> : <User size={compact ? 14 : 18} />}
      </div>
      ...
      <div className={styles.info}>
        ...
        <div className={styles.meta}>
          <span className={`${styles.roleBadge} ${styles[`role_${roleColor}`]}`}>{role}</span>
          {genderLabel && (
            <span className={`${styles.genderBadge} ${isMale ? styles.male : styles.female}`}>
              {genderLabel}
            </span>
          )}
          {member.group && <span className={styles.groupTag}>{member.group}</span>}
        </div>
      </div>
